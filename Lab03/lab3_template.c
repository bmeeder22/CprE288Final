...
//	 Function variables
	unsigned char new_value = GPIO_PORTB_DATA_R & 0b00000011;			// Step 1: Decide how to read PINC so that the push buttons are masked
	int8_t rotation = GPIO_PORTB_DATA_R;
...
int main(void) {
	...
	//TODO
	while(1){
		lcd_printf("Encoder: %d", read_shaft_encoder());
	}
}
